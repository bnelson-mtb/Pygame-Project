...
def level2():
    ...
    level_2_map = pygame.image.load("Level 2.png")
    level_2_map_size = level_2_map.get_size()
    level_2_map_rect = level_2_map.get_rect()

    # create the window based on the map size
    screen = pygame.display.set_mode(level_2_map_size)
    level_2_map = level_2_map.convert_alpha()
    level_2_map.set_colorkey((255, 255, 255))
    ...
    background_2 = pygame.transform.smoothscale(background_2, level_2_map.get_size())  # Resize to MATCH the maze
    background_2_rect = background_2.get_rect()
    background_2_mask = pygame.mask.from_surface(background_2)
    ...
    start_button_rect.center = (1036, 632)
    start_button_mask = pygame.mask.from_surface(start_button)

    # Create the seaweed
    seaweed = pygame.image.load("theseaweed.png").convert_alpha()
    seaweed = pygame.transform.smoothscale(seaweed, (40,40))
    seaweed_rect = seaweed.get_rect()
    seaweed_rect.center = (574, 437)
    seaweed_mask = pygame.mask.from_surface(seaweed)

    seaweed_2 = pygame.image.load("theseaweed.png").convert_alpha()
    seaweed_2 = pygame.transform.smoothscale(seaweed_2, (40, 40))
    seaweed_2_rect = seaweed_2.get_rect()
    seaweed_2_rect.center = (1074, 253)
    seaweed_2_mask = pygame.mask.from_surface(seaweed_2)

    # Seaweed movement attributes
    seaweed_direction = 1  # 1 for right, -1 for left
    seaweed_speed = 3  # How many pixels to move each frame
    seaweed_2_direction = 1  # 1 for right, -1 for left
    seaweed_2_speed = 3  # How many pixels to move each frame
    ...
    while is_alive:
        ...
        if started:
            ...
            if pixel_collision(player_mask, player_rect, seaweed_mask, seaweed_rect):
                return False

            if pixel_collision(player_mask, player_rect, seaweed_2_mask, seaweed_2_rect):
                return False

            # See if we touch the maze walls
            if pixel_collision(player_mask, player_rect, level_2_map_mask, level_2_map_rect):
                return False
            ...
            if seaweed_rect.left < 490 or seaweed_rect.right > 772:
                seaweed_direction *= -1  # Reverse direction

            # Check if it needs to change direction
            if seaweed_2_rect.left < 750 or seaweed_2_rect.right > 1112:
                seaweed_2_direction *= -1  # Reverse direction

            seaweed_rect.centerx += seaweed_direction * seaweed_speed
            seaweed_2_rect.centerx += seaweed_2_direction * seaweed_2_speed
        ...
        # Draw the seaweed
        screen.blit(seaweed, seaweed_rect)
        screen.blit(seaweed_2, seaweed_2_rect)
